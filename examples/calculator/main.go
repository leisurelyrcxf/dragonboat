...
func main() {
	...
	consoleStopper.RunWorker(func() {
		...
		for {
			...
			if s[0] != '+' && s[0] != '-' && s[0] != '*' && s[0] != '/' {
				fmt.Fprintf(os.Stderr, "Need arithemetic op\n")
				continue
			}
			ch <- s
		}
	...
	raftStopper.RunWorker(func() {
		...
		for {
			...
			case <-ticker.C:
				...
				if err == nil {
					var current uint64
					current = binary.LittleEndian.Uint64(result.([]byte))
...
func (s *ExampleStateMachine) Lookup(query interface{}) (interface{}, error) {
	result := make([]byte, 8)
	binary.LittleEndian.PutUint64(result, uint64(s.Value))
	return result, nil
}
...
func (s *ExampleStateMachine) Update(data []byte) (sm.Result, error) {
	...
	if data[0] != '+' && data[0] != '-' && data[0] != '*' && data[0] != '/' {
		return sm.Result{}, errors.New("invalid op")
	}
	operand, err := strconv.Atoi(strings.TrimSpace(string(data[1:])))
	if err != nil {
		return sm.Result{}, err
	}
	switch data[0] {
	case '+':
		s.Value+=int64(operand)
	case '-':
		s.Value-=int64(operand)
	case '*':
		s.Value*=int64(operand)
	case '/':
		if operand == 0 {
			return sm.Result{}, errors.New("divide by 0")
		}
		s.Value/=int64(operand)
	default:
		panic("unreachable code")
	}
	fmt.Printf("from ExampleStateMachine.Update(), msg: %s, value:%d\n",
		string(data), s.Value)
	return sm.Result{Value: uint64(len(data))}, nil
}
...
func (s *ExampleStateMachine) SaveSnapshot(w io.Writer,
	...
	// the fc untouched
	data := make([]byte, 8)
	binary.LittleEndian.PutUint64(data, uint64(s.Value))
	_, err := w.Write(data)
	return err
}
